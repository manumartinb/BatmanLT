# -*- coding: utf-8 -*-
"""
CRD SPX V37 — SISTEMA FWD (Forward Testing) con Mediana Intraday 14×30min — LOCAL DATA
===========================================================================
VERSIÓN: CRD V37 — Mediana Intraday sobre 14 timestamps fijos de mercado US
...
6) **Adaptación a estructura CRD (2 legs)**:
   - Función `batman_value_from_df`: revaloriza -1C@k1 +2C@k2 (sin k3)
...
# Carpeta de salida para resultados de backtest (se crea automáticamente)
OUTPUT_DIR = DESKTOP / "BATMAN_BACKTEST_OUTPUT_FILES"

# Archivo de log para archivos ya procesados (persiste entre ejecuciones)
PROCESSED_FILES_LOG = OUTPUT_DIR / "processed_files_history.txt"
...
FORWARD_FRACS = [0.01, 0.05, 0.25, 0.50, 0.90]  # Fracciones del DTE1 para ventanas forward
...
FWD_ON_WINNERS = True    # True: evalúa FWD en los mejores batmans (mayor valor según RANKING_MODE)
...
FWD_TOP_PCT = 1.00      #OJO ahora no elimina tantos. Deja candidato si hay distinta fecha (fix deduplicacion)
...
# Control global de generación de gráficos
ALL_PLOTS_ENABLED = True  # True: genera TODOS los gráficos PNG (promedios, filtrados, winners/losers)
                           # False: NO genera NINGÚN gráfico (ahorra tiempo y espacio en disco)
                           # Los gráficos se guardan en OUTPUT_DIR (BATMAN_BACKTEST_OUTPUT_FILES)
...
RANKING_MODE = "BQI_V2_ABS"              # Métrica de ranking para ordenar estructuras
                                          # "BQI_V2_ABS": Batman Quality Index V2 (basado en análisis estadístico 1,271 ops)
                                          #               Combina 7 variables con pesos por correlación (rango 0-100)
                                          #               RECOMENDADO - Mayor poder predictivo
                                          # "BQI_ABS": Batman Quality Index Absoluto (balance spread/riesgo) - Versión original
...
FILTER_RATIO_BATMAN_THRESHOLD = 99999999  # Umbral para gráficos de RATIO_BATMAN: (K3-K1) / |net_credit|
...
FILTER_FF_ATM_THRESHOLD = 99999999         # Umbral para gráficos de FF_ATM (Forward Factor ATM)
...
FILTER_FF_BAT_THRESHOLD = 99999999         # Umbral para gráficos de FF_BAT (Forward Factor Batman)
...
# ================== CONFIG SISTEMA DE RECUPERACIÓN ==================
RECOVERY_MODE_ENABLED = False  # True: activa el sistema de recuperación automática de directorios huérfanos
                              # False: desactiva la recuperación, procesa normalmente sin preguntar
                              # El sistema busca directorios temp_batman_* en el escritorio y permite
                              # recuperar procesamiento interrumpido desde checkpoint o consolidated
...
BASE_K1_DELTA_MIN = 0.40     # Delta mínimo para K1 (50%)
BASE_K1_DELTA_MAX = 0.65     # Delta máximo para K1 (60%)
...
K2_DELTA_MIN = 0.20          # Delta mínimo para K2 (28%)
K2_DELTA_MAX = 0.40          # Delta máximo para K2 (35%)
...
# === K3 REMOVED for CRD structure (2-leg strategy) ===
# CRD usa solo 2 patas: -1C@K1(exp1) + 2C@K2(exp2)
# Sin necesidad de K3 (upper sold call leg)

# === Comentarios de referencia ===
# Configuración conservadora: BASE_K1=[0.45,0.55], K2=[0.25,0.40] (cerca de ATM)
# Configuración agresiva:     BASE_K1=[0.30,0.50], K2=[0.10,0.30] (más OTM)
# NOTA: Estructura CRD necesita solo 2 strikes: K1 < K2
...
PREFILTER_CREDIT_MAX = 0    # Crédito máximo aceptable: -2 pts = recibir máximo $200 por contrato
...
# === FILTRO RATIO_UEL_EARS ===
RATIO_UEL_EARS_MIN = -0.5      # Ratio mínimo UEL / Promedio(EarL, EarR)
...
BQI_ABS_MIN = 0                       # BQI_ABS mínimo aceptable (métrica de calidad estructura)
...
# Variables globales para acumular progreso entre rondas
_PROGRESO_EXITOSOS_ACUMULADOS = 0
_PROGRESO_FALLIDOS_ACUMULADOS = 0
_PROGRESO_CARGADO = False  # Bandera para saber si ya se cargó el archivo existente

# ================== HELPERS ==================
def cargar_progreso_existente(progress_file: Path) -> tuple[int, int]:
    """
    Lee los contadores del archivo batman_progress.txt existente (si existe).

    Args:
        progress_file: Path al archivo de progreso

    Returns:
        tuple[int, int]: (exitosos, fallidos)
    """
    try:
        ...
        # Buscar contadores usando regex
        import re
        match_exitosos = re.search(r'✓ Exitosos:\s+(\d+)', content)
        match_fallidos = re.search(r'✗ Fallidos:\s+(\d+)', content)

        exitosos = int(match_exitosos.group(1)) if match_exitosos else 0
        fallidos = int(match_fallidos.group(1)) if match_fallidos else 0

        if exitosos > 0 or fallidos > 0:
            print(f"[PROGRESO] Cargando progreso previo: {exitosos} exitosos, {fallidos} fallidos")

        return (exitosos, fallidos)

    except Exception as e:
        print(f"[AVISO] Error al cargar progreso existente: {e}")
        return (0, 0)

def actualizar_archivo_progreso(exitosos_ronda: int, fallidos_ronda: int, output_dir: Path):
    """
    Actualiza un archivo TXT con el progreso acumulado de la ejecución del script.

    Simplemente acumula los exitosos y fallidos de la ronda actual a los totales globales.
    Si el script se reinicia, carga automáticamente el progreso del archivo existente.

    Args:
        exitosos_ronda: Número de tareas completadas exitosamente en esta ronda
        fallidos_ronda: Número de tareas fallidas en esta ronda
        output_dir: Directorio donde guardar el archivo de progreso
    """
    global _PROGRESO_EXITOSOS_ACUMULADOS, _PROGRESO_FALLIDOS_ACUMULADOS, _PROGRESO_CARGADO

    try:
        # Primera vez: cargar progreso del archivo existente (si existe)
        if not _PROGRESO_CARGADO:
            output_dir.mkdir(parents=True, exist_ok=True)
            progress_file = output_dir / "batman_progress.txt"

            exitosos_prev, fallidos_prev = cargar_progreso_existente(progress_file)
            _PROGRESO_EXITOSOS_ACUMULADOS = exitosos_prev
            _PROGRESO_FALLIDOS_ACUMULADOS = fallidos_prev
            _PROGRESO_CARGADO = True

        # Acumular los resultados de esta ronda
        _PROGRESO_EXITOSOS_ACUMULADOS += exitosos_ronda
        _PROGRESO_FALLIDOS_ACUMULADOS += fallidos_ronda

        # Calcular totales
        total_procesados = _PROGRESO_EXITOSOS_ACUMULADOS + _PROGRESO_FALLIDOS_ACUMULADOS
        tasa_exito = (_PROGRESO_EXITOSOS_ACUMULADOS / total_procesados * 100) if total_procesados > 0 else 0

        # Crear el contenido del archivo
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        content = f"""╔═══════════════════════════════════════════════════════════╗
║     BATMAN V33 - PROGRESO ACUMULADO                      ║
╚═══════════════════════════════════════════════════════════╝
...
RESUMEN ACUMULADO:
────────────────────────────────────────────────────────────
  ✓ Exitosos: {_PROGRESO_EXITOSOS_ACUMULADOS}
  ✗ Fallidos: {_PROGRESO_FALLIDOS_ACUMULADOS}
────────────────────────────────────────────────────────────
...
"""
        ...
        # No interrumpir el proceso si falla la escritura del archivo
        print(f"[AVISO] Error al actualizar archivo de progreso: {e}")
...
def list_local_files(folder: str) -> list[Path]:
    """
    Lista archivos 30MIN*.csv en la carpeta especificada, ordenados cronológicamente.

    Args:
        folder: Carpeta donde buscar archivos

    Returns:
        Lista ordenada cronológicamente de Paths de archivos encontrados
    """
    import re
    ...
    # Buscar archivos en carpeta principal
    files = list(p.glob("30MIN*.csv"))
    ...
    # Ordenar cronológicamente por fecha en el nombre
    files_sorted = sorted([f for f in files if f.is_file()], key=extract_date)
    return files_sorted

# ================== GESTIÓN DE ARCHIVOS PROCESADOS ==================
def load_processed_files(log_path: Path) -> set:
    """
    Lee el archivo de log de archivos procesados y retorna un set de nombres.

    Args:
        log_path: Path al archivo de log

    Returns:
        Set con nombres de archivos ya procesados
    """
    ...
    with open(log_path, 'r', encoding='utf-8') as f:
        for line in f:
            # Extraer solo el nombre del archivo (antes del '|' si hay timestamp)
            filename = line.strip().split('|')[0].strip()
            if filename:
                processed.add(filename)
    return processed

def append_processed_files(log_path: Path, filenames: list):
    """
    Añade archivos procesados al log (append mode) con timestamp.
    ...
    # Usar la misma zona horaria que el resto del sistema
    TZ_ES = ZoneInfo("Europe/Madrid")
    timestamp = datetime.now(TZ_ES).strftime("%Y-%m-%d %H:%M:%S")
...
# gen_k3_candidates_by_delta REMOVED for CRD structure (no K3 leg needed)
...
# ================== URL ==================
def make_url(exp1,k1,exp2,k2,k3=None):
    """
    Genera URL de la estructura.
    CRD: solo 2 legs (k3=None), Batman: 3 legs (k3 != None)
    """
...
def batman_value_from_df(one_min_df: pd.DataFrame, exp1: str, k1: float, exp2: str, k2: float, k3=None, root1=None, root2=None, current_date_str=None):
    """
    Revaloriza estructura CRD: -1C@k1(exp1) +2C@k2(exp2) (2 legs)
    # k3: DEPRECADO en CRD - mantener por compatibilidad pero no se usa
    ...
    Returns:
        dict con net_credit (puntos SPX), leg1/leg2 (bid/ask/mid/root/strike/iv),
    ...
    try:
        ...
        # Buscar strike k1 en exp1 (CRD: solo 2 patas, sin k3)
        ...
        def _build_leg(row, idx, mid, tie_break, root_pref_applied, root_chosen, exp_root):
            ...
            return {
                ...
                "root_expected": normalize_root_value(exp_root)
            }
        ...
        # net_credit = -mid1 + 2*mid2 (puntos SPX) - CRD: solo 2 patas
        ...
        if current_date_str is not None:
            try:
                ...
                if np.isfinite(spot) and spot > 0:
                    ...
                    # Agregar IVs a las patas (CRD: solo 2 patas)
                    leg1["iv"] = iv1
                    leg2["iv"] = iv2
...
def compute_strategy_metrics(spot,r_base,exp1,dte1,k1,exp2,dte2,k2,k3=None,precache=None):
    ...
    # d3, t3, p3, iv3 REMOVED for CRD (no K3 leg)

    dz = lambda x: 0.0 if (x is None or (isinstance(x,float) and math.isnan(x))) else x
    delta_total = -1*dz(d1) + 2*dz(d2)  # CRD: solo 2 patas
    theta_total = -1*dz(t1) + 2*dz(t2)  # CRD: solo 2 patas

    p1z, p2z = dz(p1), dz(p2)
    net_credit = (-1*p1z) + (2*p2z)  # puntos SPX - CRD: solo 2 patas
    ...
    # pnl_short3 REMOVED for CRD (no K3 leg)
    pnl_total  = pnl_short1 + pnl_long2  # CRD: solo 2 patas

    # Death Valley + PnLDV (puntos SPX) - CRD simplificado (solo K1, K2)
    death_valley = None
    pnl_dv_points = None
    tau = max(T2 - T1, 0.0)
    if tau > 0 and (iv2 is not None) and not (isinstance(iv2,float) and math.isnan(iv2)):
        ...
        # CRD: death valley simplificado, solo comparamos con K1 (sin K3)
        if S0 >= float(k1):
            ...
            value_lim = float(k1) - 2.0*float(k2)*math.exp(-r2*tau)
            pnl_dv_points = value_lim - net_credit
            death_valley  = float('nan')

    # Orejas (puntos SPX) - CRD: solo EarL (sin EarR)
    ...
    # LEL (Low Expiration Line) - CRD específico
    LEL = -net_credit

    # UEL_inf_USD REMOVED for CRD (no K3 leg)
    # RATIO_UEL_EARS REMOVED for CRD (no K3 leg)
    ...
    # iv3_fmt REMOVED for CRD (no K3 leg)

    # Calcular FF_ATM (Forward Factor)
    # Para CRD: ATM_front = IV de k1 (solo 1 wing), ATM_back = IV de body (k2)
    # Identificador de fila para logging (opcional)
    row_id = f"{exp1}/{exp2}|{k1}/{k2}"

    # Calcular ATM_front = IV de K1 (CRD: solo 1 wing)
    ...
    # FF_BAT REMOVED for CRD (específico de Batman con K3)
    # RATIO_BATMAN REMOVED for CRD (específico de Batman con K3)
    ...
    out = {
        ...
        "k1": int(k1), "k2": int(k2),  # CRD: solo 2 strikes (sin k3)

        "iv_k1": iv1_fmt,
        "iv_k2": iv2_fmt,
        # iv_k3 REMOVED for CRD

        "FF_ATM": ff_atm_val if np.isfinite(ff_atm_val) else None,
        # FF_BAT REMOVED for CRD
        # RATIO_BATMAN REMOVED for CRD
        ...
        # EarR REMOVED for CRD
        "LEL": round(LEL, 2) if np.isfinite(LEL) else None,  # CRD: Low Expiration Line
        ...
        # delta_k3, theta_k3 REMOVED for CRD
        ...
        # price_bid_short3, price_ask_short3, price_last_short3, price_mid_short3 REMOVED for CRD

        "pnl8000_short1": round(pnl_short1,2),
        "pnl8000_long2":  round(pnl_long2,2),
        # pnl8000_short3 REMOVED for CRD
        "pnl8000_total":  round(pnl_total,2),

        # UEL_inf_USD REMOVED for CRD
        # RATIO_UEL_EARS REMOVED for CRD
    }
    return out
...
def _process_one_fwd_batman(args):
    ...
    result = {
        ...
        'fwd_data': {},
        'errors': [],
        'failure_reason': None  # Razón principal de fallo
    }
    ...
    try:
        ...
        result['failure_reason'] = f"ERROR: No se pudo parsear DTE1/DTE2='{dte_str}'"
        return result

    if not t1_days or t1_days <= 0:
        result['errors'].append(('fwd_invalid_dte', f'DTE1={t1_days}'))
        result['failure_reason'] = f"ERROR: DTE1 inválido ({t1_days} días)"
        return result
    ...
    for fr, suf in zip(forward_fracs, frac_suffixes):
        ...
        if fwd_path is None:
            missing_files.append(f"{suf}(índice fuera de rango)")
            continue
        if not fwd_path.exists():
            missing_files.append(f"{suf}(archivo no existe)")
            continue

        try:
            m2 = re.search(r"(\d{4}-\d{2}-\d{2})", fwd_path.name)
            if not m2:
                result['errors'].append(('fwd_filename_parse_error', f'{suf}: {fwd_path.name}'))
                continue
            ...
            if fwd_df.empty:
                empty_files.append(f"{suf}({fwd_path.name})")
                continue
            ...
            for ts_hhmm in intraday_timestamps:
                ...
                # ============================================================
                # PnLDV FORWARD REMOVED for CRD (específico de Batman con K3)
                # ============================================================
                # CRD: sin compute_pnldv_forward (específico de Batman)
                ...
                if first_valid_timestamp is None:
                    ...
                    # Metadata de las 2 patas (CRD: solo k1, k2)
                    for leg_label, leg_info in (("k1", bv_details.get("leg1")),
                        ...
                        # Validación de mid (CRD: solo k1, k2)
                        if leg_label == "k1":
            ...
            # PnLDV forward mediana REMOVED for CRD
            ...
            if first_valid_timestamp is not None:
                ...
                # No se pudo calcular mediana para esta ventana
                if timestamps_found == 0:
                    no_timestamps.append(f"{suf}(0/14 timestamps)")
                elif timestamps_revalued == 0:
                    revaluation_failures.append(f"{suf}({timestamps_found}/14 encontrados, 0 revalorizados)")
                else:
                    revaluation_failures.append(f"{suf}({timestamps_revalued}/14 revalorizados, insuficientes)")
    ...
    # Generar razón de fallo si no hubo éxito
    if not result['success']:
        reasons = []
        if missing_files:
            reasons.append(f"Archivos no encontrados: {', '.join(missing_files)}")
        if empty_files:
            reasons.append(f"Archivos vacíos: {', '.join(empty_files)}")
        if no_timestamps:
            reasons.append(f"Sin timestamps: {', '.join(no_timestamps)}")
        if revaluation_failures:
            reasons.append(f"Fallos revalorización: {', '.join(revaluation_failures)}")

        window_errors = [e for e in result['errors'] if e[0] == 'fwd_window_error']
        if window_errors:
            reasons.append(f"Excepciones: {len(window_errors)} ventana(s)")

        if reasons:
            result['failure_reason'] = " | ".join(reasons)
        else:
            result['failure_reason'] = "Sin datos FWD procesables en ninguna ventana"
...
def _process_k1_candidate(args):
    ...
    Retorna lista de candidatos CRD generados para este k1 (2 legs, sin K3).
    """
    ...
    # Loop k2 (CRD: solo 2 patas, sin loop k3)
    for k2 in s2_list:
        ...
        # CRD: sin loop k3 (solo 2 patas)
        # Prefiltro de crédito
        pre_net_credit = (-mid1) + (2*mid2)  # CRD: solo 2 patas
        if not (PREFILTER_CREDIT_MIN <= pre_net_credit <= PREFILTER_CREDIT_MAX):
            continue

        # Validar orden de strikes
        if not (k1 < k2):  # CRD: solo 2 strikes
            continue

        # Generar URL (k3=None para CRD)
        url = make_url(e1, k1, e2, k2, None)
        ...
        # Computar métricas (k3=None para CRD)
        ...
        # Inicializar columnas FWD vacías (incluye medianas) - CRD: solo k1, k2
        fwd_cols = {}
        for suf in FRAC_SUFFIXES:
            ...
            for leg in ("k1", "k2"):  # CRD: solo 2 patas
...
def main():
    ...
    if RECOVERY_MODE_ENABLED:
                ...
                if consolidated_file.exists() or checkpoint_file.exists():
                    ...
                    while True:
                        ...
                        if response == "info":
                            ...
                            batch_out_name = f"Batman_V18_BACKTEST_LOCAL_{ts_batch}_RECOVERED.csv"
                            batch_out_path = OUTPUT_DIR / safe_filename(batch_out_name)
        ...
        # Si RECOVERY_MODE_ENABLED es False, saltar directamente a procesamiento normal
        print(f"\n[INFO] Sistema de recuperación desactivado (RECOVERY_MODE_ENABLED = False)")
        RECOVERY_MODE = False
    ...
    if 'RECOVERY_MODE' not in locals() or not RECOVERY_MODE:
        ...
        # Obtener lista completa de archivos (ordenada cronológicamente)
        files_all = list_local_files(DATA_DIR)
        if not files_all:
            print(f"[×] No se encontraron 30MIN*.csv en: {DATA_DIR}")
            return []

        # Cargar archivos ya procesados desde el log
        processed_files = load_processed_files(PROCESSED_FILES_LOG)

        # Filtrar archivos no procesados
        files_available = [f for f in files_all if f.name not in processed_files]

        if not files_available:
            print(f"[×] Todos los archivos ya han sido procesados ({len(files_all)} totales)")
            print(f"[i] Para reprocesar, elimina o edita: {PROCESSED_FILES_LOG}")
            return []

        # Seleccionar archivos aleatoriamente de los disponibles
        k = min(NUM_RANDOM_FILES, len(files_available))
        chosen_files = random.sample(files_available, k=k)

        # Para FWD, usar la lista completa (ordenada cronológicamente)
        files_sorted = files_all

        print(f"[i] Archivos totales en directorio: {len(files_all)}")
        print(f"[i] Archivos ya procesados: {len(processed_files)}")
        print(f"[i] Archivos disponibles para selección: {len(files_available)}")
        print(f"[i] Archivos seleccionados esta ronda: {k}")
        print(f"[i] Log de archivos procesados: {PROCESSED_FILES_LOG}")

        ts_batch = datetime.now(TZ_ES).strftime("%Y%m%d_%H%M%S")
        batch_out_name = f"Batman_V18_BACKTEST_LOCAL_{ts_batch}.csv"
        batch_out_path = OUTPUT_DIR / safe_filename(batch_out_name)
            ...
            for t_idx, target_hhmm_us in enumerate(TARGET_HHMMS_US if not IGNORE_TARGET_MINUTE else ["ALL"], start=1):
                ...
                for e1 in exp_A:
                    ...
                    for e2 in exp_B:
                        ...
                        # Crear tareas para cada k1 (CRD: sin s3_list)
                        for k1 in s1_list:
                            task = (
        ...
        for pq_file in parquet_files:
            ...
            if not df_chunk.empty and {"PnLDV","EarL","EarR"}.issubset(df_chunk.columns):
                if "BQI_ABS" not in df_chunk.columns:
                    ...
                    # ========== BQI_V2_ABS - Versión mejorada basada en análisis estadístico ==========
                    # Basado en correlaciones de 1,271 operaciones (ver INFORME_ANALISIS_ESTADISTICO.md)
                    # Combina las 7 variables más correlacionadas con rentabilidad

                    # 1. EarScore ya calculado (corr: +0.1996, peso: 30%)
                    EarScore_norm = np.clip(EarScore / 142.42, 0, 1)

                    # 2. delta_total (corr: +0.1493, peso: 20%)
                    delta_total_val = df_chunk["delta_total"].fillna(0).values
                    delta_total_norm = np.clip(delta_total_val / 0.10, 0, 1)

                    # 3. net_credit penalty (corr: -0.1045, peso: 10% inverso)
                    # Más negativo = mejor (paradoja del crédito)
                    net_credit_val = df_chunk["net_credit"].fillna(0).values
                    net_credit_penalty = np.clip((-net_credit_val - 0.05) / 40.0, 0, 1)

                    # 4. PnLDV (corr: +0.0980, peso: 15%)
                    PnLDV_norm = np.clip((pnldv + 78) / 187.0, 0, 1)

                    # 5. theta_total (corr: +0.0866, peso: 15%)
                    theta_total_val = df_chunk["theta_total"].fillna(0).values
                    theta_total_norm = np.clip(theta_total_val / 0.60, 0, 1)

                    # 6. BQI_ABS normalizado (corr: +0.0923, peso: 10%)
                    # Limpiar offset de 1000 y normalizar
                    BQI_ABS_clean = np.where(BQI_ABS > 100, BQI_ABS / 1000.0, BQI_ABS)
                    BQI_ABS_norm = np.clip(BQI_ABS_clean / 20.0, 0, 1)

                    # 7. RATIO_BATMAN penalty (corr: -0.0512, peso: 5% inverso)
                    # Valores bajos = mejor
                    RATIO_BATMAN_val = df_chunk["RATIO_BATMAN"].fillna(8900).values
                    RATIO_BATMAN_penalty = np.clip(1.0 - (RATIO_BATMAN_val / 8900.0), 0, 1)

                    # Combinar con pesos basados en fuerza de correlación
                    BQI_V2_ABS = (
                        0.30 * EarScore_norm +
                        0.20 * delta_total_norm +
                        0.15 * PnLDV_norm +
                        0.15 * theta_total_norm +
                        0.10 * BQI_ABS_norm +
                        0.10 * net_credit_penalty +
                        0.05 * RATIO_BATMAN_penalty
                    ) * 100

                    df_chunk["BQI_V2_ABS"] = pd.to_numeric(BQI_V2_ABS, errors="coerce")

                    # Liberar memoria
                    del earL, earR, pnldv, EL, ER, EarScore, ValleyDepth, Asym, BQI_ABS_pos, BQI_ABS_neg, BQI_ABS
                    del EarScore_norm, delta_total_val, delta_total_norm, net_credit_val, net_credit_penalty
                    del PnLDV_norm, theta_total_val, theta_total_norm, BQI_ABS_clean, BQI_ABS_norm
                    del RATIO_BATMAN_val, RATIO_BATMAN_penalty, BQI_V2_ABS
    ...
    if df.empty:
        ...
        # Usar RANKING_MODE para determinar criterio de ordenamiento
        choice = RANKING_MODE  # Puede ser: "BQI_V2_ABS", "BQI_ABS", "PnLDV", "EarScore", etc.
        ...
        try:
            ...
            if choice == "BQI_V2_ABS":
                # BQI_V2_ABS: Versión mejorada con tie-breakers secundarios
                sort_cols = ["BQI_V2_ABS","EarScore","delta_total","PnLDV"]
                sort_asc = [False,False,False,False]
                used = "BQI_V2_ABS → EarScore → delta_total → PnLDV"
            elif choice in ("theta", "t", "θ", "theta_total"):
    ...
    if not df.empty and (FWD_ON_WINNERS or FWD_ON_LOSERS):
        ...
        if fwd_indices_to_process:
            # ============================================================
            # FIX: Resetear índices del DataFrame para evitar duplicados
            # ============================================================
            # El DataFrame puede tener índices duplicados de múltiples archivos CSV.
            # Cuando hacemos df.loc[idx] con un índice duplicado, Pandas devuelve
            # múltiples filas como una Serie, rompiendo el procesamiento paralelo.
            #
            # Solución: Mapear índices antiguos → posiciones, resetear, actualizar lista
            # ============================================================

            print(f"[FWD] Verificando y corrigiendo índices duplicados en DataFrame...")

            # Detectar si hay índices duplicados
            unique_indices = df.index.nunique()
            total_rows = len(df)
            if unique_indices < total_rows:
                print(f"[FWD] ⚠️ DETECTADO: DataFrame tiene {total_rows - unique_indices} índices duplicados")
                print(f"[FWD]    Total filas: {total_rows}, Índices únicos: {unique_indices}")

            # Mapear fwd_indices_to_process a posiciones
            # Para cada índice que queremos procesar, encontrar su(s) posición(es) en el DataFrame
            fwd_positions = []
            for old_idx in fwd_indices_to_process:
                # Encontrar TODAS las posiciones que tienen este índice
                positions = [pos for pos, idx in enumerate(df.index) if idx == old_idx]

                if not positions:
                    print(f"[FWD] ⚠️ Advertencia: Índice {old_idx} no encontrado en DataFrame")
                elif len(positions) > 1:
                    # Índice duplicado: usar la primera ocurrencia
                    print(f"[FWD] ⚠️ Índice {old_idx} duplicado ({len(positions)} veces). Usando primera ocurrencia (posición {positions[0]})")
                    fwd_positions.append(positions[0])
                else:
                    # Índice único: usar esa posición
                    fwd_positions.append(positions[0])

            # Resetear índice del DataFrame (genera nuevos índices 0,1,2,3...)
            df = df.reset_index(drop=True)

            # Actualizar lista de índices a procesar con las nuevas posiciones
            fwd_indices_to_process = fwd_positions

            print(f"[FWD] ✓ Índices reseteados. Procesando {len(fwd_indices_to_process)} batmans con índices únicos.")
            print()

            # Preparar argumentos para workers
            worker_args = []
            for idx in fwd_indices_to_process:
                row = df.loc[idx]  # Ahora idx es único y válido
            ...
            completed_count = 0
            failed_count = 0
            failure_reasons = []  # Lista de tuplas (idx, reason)
            timeout_count = 0
                ...
                for future in futures:
                    ...
                    try:
                        ...
                        if result['success']:
                            ...
                            failed_count += 1
                            reason = result.get('failure_reason', 'Sin razón específica')
                            failure_reasons.append((idx, reason))

                    except TimeoutError:
                        timeout_count += 1
                        failed_count += 1
                        failure_reasons.append((idx, "TIMEOUT: Procesamiento excedió 60 segundos"))
                    except Exception as e:
                        print(f"[FWD] Error procesando índice {idx}: {e}")
                        failed_count += 1
                        failure_reasons.append((idx, f"EXCEPCIÓN: {str(e)[:100]}"))

            print(f"\n[FWD INTRADAY MEDIANA 14×30min] Cálculo completado:")
            print(f"  ✓ Exitosos: {completed_count}")
            print(f"  ✗ Fallidos: {failed_count}")

            # Actualizar archivo de progreso acumulado
            actualizar_archivo_progreso(completed_count, failed_count, OUTPUT_DIR)

            # Mostrar desglose detallado de fallos
            if failure_reasons:
                print(f"\n  → MOTIVOS DE FALLOS ({len(failure_reasons)} registros):")

                # Agrupar por tipo de error
                error_categories = {}
                for idx, reason in failure_reasons:
                    # Extraer categoría principal (primera parte antes de ':')
                    if "TIMEOUT" in reason:
                        category = "TIMEOUT (>60s)"
                    elif "Archivos no encontrados" in reason:
                        category = "Archivos forward no encontrados"
                    elif "Sin timestamps" in reason:
                        category = "Timestamps no encontrados en CSV"
                    elif "Fallos revalorización" in reason:
                        category = "Fallo al revalorizar estructura Batman"
                    elif "Archivos vacíos" in reason:
                        category = "CSV forward vacío"
                    elif "EXCEPCIÓN" in reason:
                        category = "Excepción durante procesamiento"
                    else:
                        category = "Otros errores"

                    if category not in error_categories:
                        error_categories[category] = []
                    error_categories[category].append((idx, reason))

                # Mostrar resumen por categoría
                for category, items in sorted(error_categories.items(), key=lambda x: -len(x[1])):
                    print(f"\n     [{category}]: {len(items)} caso(s)")
                    # Mostrar primeros 5 ejemplos
                    for idx, reason in items[:5]:
                        # Obtener info del Batman
                        batman_info = ""
                        if idx in df.index:
                            dia = df.at[idx, 'dia'] if 'dia' in df.columns else 'N/A'
                            dte = df.at[idx, 'DTE1/DTE2'] if 'DTE1/DTE2' in df.columns else 'N/A'
                            batman_info = f"(dia={dia}, DTE={dte})"
                        print(f"       - Índice {idx} {batman_info}: {reason}")
                    if len(items) > 5:
                        print(f"       ... y {len(items)-5} casos más")

            print(f"\n  Columnas generadas: PnL_fwd_pts_*_mediana, PnL_fwd_pct_*_mediana")
            print(f"{'='*70}\n")
    ...
    if not df.empty:
        preferred_order = [
            ...
            # IVs y Forward Factor
            "iv_k1", "iv_k2", "iv_k3", "FF_ATM", "FF_BAT",
    ...
    if col_filter not in df_copy.columns:
        ...
        if len(df_copy) > 0:
            ...
            batch_out_path_copy = OUTPUT_DIR / safe_filename(batch_out_name_copy)
            ...
            # ============================================================
            # GENERACIÓN DE GRÁFICOS (controlado por ALL_PLOTS_ENABLED)
            # ============================================================
            if ALL_PLOTS_ENABLED:
                ...
                if len(pnl_cols_sorted) >= 1 and len(df_copy) > 0:
                    ...
                    if len(promedios) >= 1:
                        ...
                        plot_path = OUTPUT_DIR / safe_filename(plot_name)
                ...
                if len(pnl_pts_cols_sorted) >= 1 and len(df_copy) > 0:
                    ...
                    if len(promedios_pts) >= 1:
                        ...
                        plot_path_pts = OUTPUT_DIR / safe_filename(plot_name_pts)
                    ...
                    if n_ratio > 0:
                        ...
                        if len(pnl_pct_cols) > 0:
                            ...
                            if len(promedios_ratio_pct) > 0:
                                ...
                                plot_path_ratio_pct = OUTPUT_DIR / plot_name_ratio_pct
                        ...
                        if len(pnl_pts_cols) > 0:
                            ...
                            if len(promedios_ratio_pts) > 0:
                                ...
                                plot_path_ratio_pts = OUTPUT_DIR / plot_name_ratio_pts
                    ...
                    if n_bqi > 0:
                        ...
                        if len(pnl_pct_cols) > 0:
                            ...
                            if len(promedios_bqi_pct) > 0:
                                ...
                                plot_path_bqi_pct = OUTPUT_DIR / plot_name_bqi_pct
                        ...
                        if len(pnl_pts_cols) > 0:
                            ...
                            if len(promedios_bqi_pts) > 0:
                                ...
                                plot_path_bqi_pts = OUTPUT_DIR / plot_name_bqi_pts
                    ...
                    if n_ff > 0:
                        ...
                        if len(pnl_pct_cols) > 0:
                            ...
                            if len(promedios_ff_pct) > 0:
                                ...
                                plot_path_ff_pct = OUTPUT_DIR / plot_name_ff_pct
                        ...
                        if len(pnl_pts_cols) > 0:
                            ...
                            if len(promedios_ff_pts) > 0:
                                ...
                                plot_path_ff_pts = OUTPUT_DIR / plot_name_ff_pts
                    ...
                    if n_ff_bat > 0:
                        ...
                        if len(pnl_pct_cols) > 0:
                            ...
                            if len(promedios_ff_bat_pct) > 0:
                                ...
                                plot_path_ff_bat_pct = OUTPUT_DIR / plot_name_ff_bat_pct
                        ...
                        if len(pnl_pts_cols) > 0:
                            ...
                            if len(promedios_ff_bat_pts) > 0:
                                ...
                                plot_path_ff_bat_pts = OUTPUT_DIR / plot_name_ff_bat_pts
                ...
                if (FWD_ON_WINNERS or FWD_ON_LOSERS) and (len(winner_indices_copy) > 0 or len(loser_indices_copy) > 0):
                    ...
                    if len(pnl_cols_mediana_sorted) >= 1:
                        ...
                        plot_path_wl = OUTPUT_DIR / safe_filename(plot_name_wl)
                        ...
                        if len(pnl_pts_cols_mediana_sorted) >= 1:
                            ...
                            plot_path_wl_pts = OUTPUT_DIR / safe_filename(plot_name_wl_pts)
                ...
                print("\n" + "="*80)
                print("[INFO] GENERACIÓN DE GRÁFICOS DESACTIVADA (ALL_PLOTS_ENABLED = False)")
                print("="*80 + "\n")
        else:
    ...
    # Retornar archivos procesados para registrarlos en el log
    if 'chosen_files' in locals():
        return chosen_files
    else:
        return []
...
if __name__=="__main__":
    # ============================================================
    # LOOP CONTINUO: Procesar archivos hasta que no queden más
    # ============================================================
    ...
    print(f"\n{'='*80}")
    print("BATMAN V30 - MODO STREAMING CONTINUO")
    print(f"{'='*80}")
    print(f"Directorio de datos: {DATA_DIR}")
    print(f"Directorio de salida: {OUTPUT_DIR}")
    print(f"Log de archivos procesados: {PROCESSED_FILES_LOG}")
    print(f"Tamaño de lote: {NUM_RANDOM_FILES} archivos por ronda")
    print(f"{'='*80}\n")
    ...
    while True:
        # Verificar archivos disponibles (excluyendo los ya procesados)
        all_files = list_local_files(DATA_DIR)
        processed_set = load_processed_files(PROCESSED_FILES_LOG)
        available_files = [f for f in all_files if f.name not in processed_set]

        if not available_files:
            print(f"\n{'='*80}")
            print("PROCESO COMPLETADO!")
            print(f"{'='*80}")
            print(f"Total de rondas procesadas: {batch_counter}")
            print(f"Total de archivos procesados: {total_files_processed}")
            print(f"Todos los archivos 30MIN*.csv han sido procesados")
            print(f"Log de procesados: {PROCESSED_FILES_LOG}")
        ...
        print(f"\n{'#'*80}")
        print(f"RONDA #{batch_counter}")
        print(f"{'#'*80}")
        print(f"Archivos totales: {len(all_files)}")
        print(f"Archivos ya procesados: {len(processed_set)}")
        print(f"Archivos disponibles: {len(available_files)}")
        print(f"Archivos a procesar esta ronda: {min(NUM_RANDOM_FILES, len(available_files))}")
        print(f"{'#'*80}\n")
        ...
        try:
            ...
            if processed_files:
                # Registrar archivos procesados en el log
                print(f"\n{'='*80}")
                print(f"REGISTRANDO ARCHIVOS PROCESADOS EN LOG")
                print(f"{'='*80}")

                filenames_to_log = [f.name for f in processed_files]
                append_processed_files(PROCESSED_FILES_LOG, filenames_to_log)
                total_files_processed += len(processed_files)

                for filename in filenames_to_log:
                    print(f"  ✓ Registrado: {filename}")

                print(f"{'='*80}")
                print(f"OK Ronda #{batch_counter} completada exitosamente")
                print(f"  Archivos procesados en esta ronda: {len(processed_files)}")
                print(f"  Total acumulado: {total_files_processed}")
                print(f"{'='*80}\n")
            else:
                print(f"\n[!] ADVERTENCIA: No se procesaron archivos en la ronda #{batch_counter}")
                print("    El script continuará con la siguiente ronda...\n")

        except KeyboardInterrupt:
            print(f"\n\n{'='*80}")
            print("PROCESO INTERRUMPIDO POR EL USUARIO")
            print(f"{'='*80}")
            print(f"Rondas completadas: {batch_counter - 1}")
            ...
            print(f"\n{'!'*80}")
            print(f"ERROR EN RONDA #{batch_counter}")
            print(f"{'!'*80}")
            print(f"Error: {e}")
            print(f"El script continuará con la siguiente ronda...")
            print(f"{'!'*80}\n")
            import traceback
            traceback.print_exc()
            continue
