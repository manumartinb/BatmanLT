...
CALL RATIO DIAGONAL (CRD) SPX V1 — SISTEMA FWDss (Forward Testing) con Mediana Intraday — LOCAL DATA
===========================================================================
VERSIÓN: CRD V9 — Basado en Batman V8, adaptado a Call Ratio Diagonal (-1C@K1 exp1 + 2C@K2 exp2)

NOVEDADES V9:
1) **Búsqueda tolerante de archivos forward**:
   - Implementación mejorada para manejar fines de semana/feriados
   - Busca fecha exacta primero, luego fecha más cercana >= target_date
   - Tolerancia de ±5 días calendario para recuperar filas que caían en días sin mercado
   - Soluciona pérdida de ~34% de filas que ocurría cuando la fecha exacta no existía
   - Mantiene precisión de fechas de V9 mientras recupera robustez de V8
   - Log de ajustes de fecha para debugging (fwd_date_adjusted)

2) **Sistema dual-list para YEARS_TO_PROCESS**:
   - files_sorted: Lista filtrada por años para selección de archivos base
   - files_sorted_fwd: Lista completa sin filtrar para búsqueda forward
   - Permite procesar solo años específicos mientras el FWD accede a TODOS los archivos
   - Soluciona pérdida de datos FWD para CRDs de final de año (diciembre)
   - Especialmente crítico para ventanas forward largas (0.50, 0.90) que requieren archivos del año siguiente
...
2) **Sistema FWD simplificado**:
   ...
   - Filtro WINNERS/LOSERS basado en RANKING_MODE (CQI_ABS, PnLDV, EarL)
   - Aplicación de FWD_TOP_PCT sobre estructuras ya filtradas
   - **DEDUPLICACIÓN por dia + DTE1/DTE2**: elimina CRDs con mismos DTEs en el MISMO día antes de FWD
...
5) **Análisis gráfico y correlaciones**:
   ...
   - Correlaciones FWD vs métricas (CQI_ABS, PnLDV, EarL, LEL, etc.)
   - Comparativa winners vs losers

6) **Adaptación a estructura Call Ratio Diagonal**:
   - Función `batman_value_from_df`: revaloriza -1C@k1 (exp1) +2C@k2 (exp2)
   - Cálculo de net_credit forward (2 patas, 2 expiraciones)
...
MANTIENE:
- Death Valley + PnLDV + CQI_ABS (CRD Quality Indicator)
- Canonical chain robusto
- Caché de greeks
- Filtros de liquidez y spread

ESTRUCTURA CALL RATIO DIAGONAL:
- Geometría: -1 call corta@K1 (exp cercana) + 2 calls largas@K2 (exp lejana)
- Puntos clave: LEL (Low Expiration Line = net_credit), EarL (oreja izquierda), PnLDV
- Sin oreja derecha: línea asintótica creciente (no hay pérdida máxima infinita)
"""
...
DATA_DIR = r"C:\Users\Administrator\Desktop\FINAL DATA\HIST AND STREAMING DATA\UPDATED HISTORICAL DAYS"
...
IGNORE_TARGET_MINUTE = False              # Control de procesamiento completo del día
...
FORWARD_FRACS = [0.15, 0.25, 0.50, 0.90]  # ! Fracciones del DTE1 para ventanas forward
...
FWD_ON_WINNERS = True    # True: evalúa FWD en los mejores CRDs (mayor valor según RANKING_MODE)
                          # False: NO evalúa FWD en winners
FWD_ON_LOSERS = True    # True: evalúa FWD en los peores CRDs (menor valor según RANKING_MODE)
                          # False: NO evalúa FWD en losers
                          # NOTA: Puedes activar ambos para evaluar extremos (winners Y losers)

# Selección por porcentaje (controla cuántos CRDs se procesan en FWD)
...
FWD_PLOT_ENABLED = True  # True: genera gráficos PNG con promedios de PnL_fwd_pct por ventana (W01, W05, etc.)
...
RANKING_MODE = "CQI_ABS"                 # ! Métrica de ranking para ordenar estructuras
                                          # "CQI_ABS": CRD Quality Indicator Absoluto (balance PnLDV/EarL)
                                          # "PnLDV": Profit & Loss en Death Valley (peor escenario)
                                          # "EarL": Oreja izquierda (P&L en K1)
                                          # "FF_GEN": Forward Factor Generalizado (ajustado por skew)
                                          # "FF_ATM": Forward Factor ATM (backwardation pura)

# ================== UMBRALES PARA GRÁFICOS FILTRADOS (Subconjuntos de Alta Calidad) ==================
# Controla qué estructuras se incluyen en los gráficos filtrados

FILTER_CQI_ABS_THRESHOLD = 2.0        # Umbral para gráficos de CQI_ABS (CRD Quality Indicator Absoluto)
                                       # Gráficos mostrarán solo estructuras con CQI_ABS > este valor
                                       # Ejemplo: 2.0 -> solo CRDs con CQI_ABS mayor a 2.0
                                       # CQI_ABS mide el balance entre PnLDV y EarL
...
FILTER_FF_ATM_THRESHOLD = 0.2         # Umbral para gráficos de FF_ATM (Forward Factor ATM)
                                       ...
                                       # Ejemplo: 0.2 -> solo CRDs con Forward Factor ATM mayor a 0.2
                                       # NOTA: FF_ATM > 0 indica backwardation (favorable para long calendar)

# ================== CONFIG PROCESO ==================
NUM_RANDOM_FILES = 999      # ! Número de archivos CSV a procesar aleatoriamente del directorio DATA_DIR
...
YEARS_TO_PROCESS = [2023]    # ! Lista de años (YYYY) a procesar. Ej: [2019, 2021, 2024]
                             # Si está vacío [], procesa TODOS los años disponibles
                             # Solo se seleccionarán archivos 30MINDATA_YYYY-MM-DD.csv de estos años
...
STATISTICAL_ANALYSIS = True  # ! True: ejecuta análisis estadístico al finalizar el backtest
...
# Umbrales mínimos por pata (CRD):
# - FRONT (exp1): pata corta k1 (vencimiento cercano)
...
MIN_OI_FRONT  = 1              # Open Interest mínimo para opciones FRONT (k1)
MIN_VOL_FRONT = 1              # Volumen mínimo para opciones FRONT (k1)
...
# ================== CONFIGURACIÓN ESTRUCTURA CRD (Call Ratio Diagonal) ==================
...
RANGE_A = (30, 50)       # ! Rango DTE para expiration FRONT (pata corta k1)
                           # (40, 200): DTEs entre 40 días y 200 días
                           # Ejemplos alternativos:
                           # (0, 20): Short DTE - estructuras de muy corto plazo
                           # (40, 180): Long DTE acotado - estructuras de mediano plazo

RANGE_B = (40, 90)       # ! Rango DTE para expiration BACK (pata larga k2)
                           # (60, 300): DTEs entre 60 días y 300 días
                           # IMPORTANTE: RANGE_B debe ser >= RANGE_A para CRD válido
                           # Ejemplos alternativos:
...
BASE_P1_DELTA_MIN = 0.40     # ! Delta mínimo para pata corta (40 puntos básicos = 40%)
BASE_P1_DELTA_MAX = 0.60     # ! Delta máximo para pata corta (60 puntos básicos = 60%)
                             # EJEMPLO: [0.40, 0.60] permite strikes con deltas entre 40% y 60%
                             # NOTA: Rango más estrecho = menos candidatos, búsqueda más rápida

# === K2 CANDIDATOS (Strike patas largas back) - BASADO EN DELTA ===
...
P2_DELTA_MIN = 0.20          # ! Delta mínimo para patas largas (20 puntos básicos = 20%)
P2_DELTA_MAX = 0.50          # ! Delta máximo para patas largas (50 puntos básicos = 50%)
                             # EJEMPLO: [0.20, 0.50] permite strikes con deltas entre 20% y 50%
                             # NOTA: Deltas menores = strikes más OTM = más alejados del precio actual

# === Comentarios de referencia ===
# Configuración conservadora: BASE_P1=[0.45,0.55], P2=[0.25,0.45] (cerca de ATM)
# Configuración agresiva:     BASE_P1=[0.30,0.50], P2=[0.10,0.30] (más OTM)
# NOTA: Estructura CRD no necesita K3, solo 2 strikes: K1 < K2
...
THETA_MIN, THETA_MAX = -1.00, 10000   # ! Theta diario permitido (decay temporal)
                                       # -2.00: pérdida máxima por theta de -$200/día
                                       # 10000: sin límite superior (acepta theta positivo alto)

# === NOTA: UEL_INF eliminado en CRD ===
# El Call Ratio Diagonal no tiene pérdida máxima infinita en la derecha
# La línea asintótica es creciente con pendiente +1 (beneficio ilimitado en subidas extremas)
...
# === FILTRO CQI_ABS (CRD Quality Indicator Absoluto) ===
FILTER_CQI_ABS_ENABLED = True         # True: aplica filtro por CQI_ABS | False: no filtra
CQI_ABS_MIN = 0                       # ! CQI_ABS mínimo aceptable (métrica de calidad estructura)
CQI_ABS_MAX = 10000                   # CQI_ABS máximo (sin límite superior práctico)
                                       # FILTRO ACTIVO: solo estructuras con CQI_ABS >= umbral configurado
...
def list_local_files(folder: str) -> list[Path]:
    ...
    files = sorted(p.glob("30MIN*.csv"))
    uniq = sorted({f.resolve() for f in files if f.is_file()})
    return list(uniq)

def extract_year_from_filename(filepath: Path) -> Optional[int]:
    """
    Extrae el año del nombre de archivo en formato 30MINDATA_YYYY-MM-DD.csv
    Retorna el año como entero o None si no se encuentra el patrón
    """
    match = re.search(r'30MIN.*?(\d{4})-\d{2}-\d{2}', filepath.name)
    if match:
        return int(match.group(1))
    return None

def filter_files_by_years(files: list[Path], years: list[int]) -> list[Path]:
    """
    Filtra archivos para incluir solo aquellos de los años especificados.
    Si years está vacío, retorna todos los archivos sin filtrar.
    """
    if not years:
        return files
...
def gen_k1_candidates_by_delta(calls_idx, available_strikes, spot, dte, r,
                                delta_min=0.40, delta_max=0.60, q=0.0):
    """
    Genera candidatos de K1 (pata corta) filtrando strikes por rango de delta.
    ...
    Args:
        ...
        r: Risk-free rate
        delta_min: Delta mínimo aceptable (ej: 0.40 = 40%)
...
def gen_k2_candidates_by_delta(calls_idx, available_strikes, spot, dte, r,
                                delta_min=0.20, delta_max=0.50, q=0.0):
    """
    Genera candidatos de K2 (patas largas) filtrando strikes por rango de delta.
    ...
    Args:
        ...
        dte: Days to expiration
        r: Risk-free rate
        delta_min: Delta mínimo aceptable (ej: 0.20 = 20%)
        delta_max: Delta máximo aceptable (ej: 0.50 = 50%)
        q: Dividend yield (default 0.0)
...
# ================== URL ==================
def make_url(exp1,k1,exp2,k2):
    """
    Genera URL de OptionStrat para Call Ratio Diagonal: -1C@K1 (exp1) + 2C@K2 (exp2)
    """
    ...
    legs=[f".{r1}{ymd1}C{int(k1)}x-1", f".{r2}{ymd2}C{int(k2)}x2"]
...
def batman_value_from_df(one_min_df: pd.DataFrame, exp1: str, k1: float, exp2: str, k2: float, k3: float = None, root1=None, root2=None):
    """
    Revaloriza estructura Call Ratio Diagonal: -1C@k1(exp1) +2C@k2(exp2)

    Args:
        one_min_df: DataFrame con snapshot del mercado
        exp1: Expiración de k1 (front, short leg)
        k1: Strike de call corta
        exp2: Expiración de k2 (back, long leg)
        k2: Strike de call larga (×2)
        k3: DEPRECADO - mantener por compatibilidad pero no se usa
        root1: Root esperado para exp1 (SPX/SPXW)
        root2: Root esperado para exp2 (SPX/SPXW)

    Returns:
        dict con net_credit (puntos SPX), leg1/leg2 (bid/ask/mid/root/strike)
    ...
    try:
        ...
        # Buscar strike k1 en exp1 (pata corta)
        r1 = sub1_filt.loc[np.isclose(sub1_filt["strike"], float(k1), atol=1e-6)]

        # Buscar strike k2 en exp2 (pata larga ×2)
        ...
        if not np.isfinite(mid1) or not np.isfinite(mid2):
            return None
        ...
        def _build_leg(row, idx, mid, tie_break, root_pref_applied, root_chosen, expected_root):
            ...
            return {
                ...
                "root_expected": normalize_root_value(expected_root)
            }

        leg1 = _build_leg(row1, r1.index[0] if len(r1) > 0 else None, mid1, tie1, root1_pref_applied, root1_chosen, root1)
        leg2 = _build_leg(row2, r2.index[0] if len(r2) > 0 else None, mid2, tie2, root2_pref_applied, root2_chosen, root2)

        # net_credit = -mid1 + 2*mid2 (puntos SPX)
        # CREDIT negativo = cobramos (favorable)
        net_credit = float(-mid1 + 2*mid2)
...
# ================== FORWARD FACTOR GENERALIZADO (FF_GEN) ==================
def compute_FF_GEN(IV_front, IV_back, K_front, K_back, T1_years, T2_years, spot,
                   beta1=None, beta2=None, row_identifier=""):
    """
    Calcula el Forward Factor Generalizado (FF_GEN) para strikes diferentes.

    Ajusta el FF clásico por efectos de skew cuando K_front ≠ K_back.

    El FF_GEN normaliza las IVs a ATM-equivalente antes de calcular el forward factor,
    removiendo el efecto del skew de volatilidad.

    Args:
        IV_front: IV anualizada del front leg (decimal, ej. 0.35 para 35%)
        IV_back: IV anualizada del back leg (decimal, ej. 0.35 para 35%)
        K_front: Strike del front leg
        K_back: Strike del back leg
        T1_years: Tiempo al vencimiento T1 en años
        T2_years: Tiempo al vencimiento T2 en años
        spot: Precio spot del subyacente
        beta1: Pendiente del skew en T1 (opcional, default: -0.15 para equity)
        beta2: Pendiente del skew en T2 (opcional, default: -0.10 para equity)
        row_identifier: String identificador para logging (opcional)

    Returns:
        dict: {
            'FF_GEN': FF generalizado total,
            'FF_temporal': Componente temporal,
            'FF_skew': Ajuste por diferencia de strikes,
            'FF_interaction': Ajuste por term structure del skew,
            'iv_fwd': Forward volatility calculada
        }
        O None si los datos son inválidos

    Formula:
        1. Normalización a ATM-equivalente:
           iv1_atm = iv1 - β₁ · (K1/S - 1)
           iv2_atm = iv2 - β₂ · (K2/S - 1)

        2. Forward volatility (clásica):
           V_fwd = (iv2_atm² · T2 - iv1_atm² · T1) / (T2 - T1)
           σ_fwd = √V_fwd

        3. Componentes:
           FF_temporal = (iv1_atm / σ_fwd) - 1
           FF_skew = -β_avg · Δm / σ_fwd
           FF_interaction = -(β₂-β₁)/(T₂-T₁) · T₁ · Δm / σ_fwd

        4. FF_GEN = FF_temporal + FF_skew + FF_interaction

    Interpretación:
        - FF_GEN > 0.20: Oportunidad tradeable (backwardation ajustada)
        - FF_GEN ≈ FF_ATM cuando K_front = K_back
        - Ajustes por skew típicamente ±2-10% del FF total
    """
    try:
        # Validar inputs
        if IV_front is None or IV_back is None or T1_years is None or T2_years is None:
            return None
        if K_front is None or K_back is None or spot is None:
            return None
        ...
        # Validar que son números finitos
        if not all(np.isfinite([iv1, iv2, k1, k2, S, t1, t2])):
            return None

        # Validar que IVs y precios son positivos
        if iv1 <= 0 or iv2 <= 0 or k1 <= 0 or k2 <= 0 or S <= 0:
            return None
        ...
        # Usar valores por defecto para betas si no se proporcionan
        # Valores típicos para equity: skew se aplana con el tiempo
        if beta1 is None:
            beta1 = -0.15  # Near-term skew empinado
        if beta2 is None:
            beta2 = -0.10  # Far-term skew más plano

        beta1 = float(beta1)
        beta2 = float(beta2)

        # 1. Calcular moneyness
        m1 = k1 / S
        m2 = k2 / S
        delta_m = m2 - m1

        # 2. Normalizar IVs a ATM-equivalente (remover efecto del skew)
        iv1_atm = iv1 - beta1 * (m1 - 1)
        iv2_atm = iv2 - beta2 * (m2 - 1)

        # 3. Calcular forward volatility usando IVs normalizados
        v1 = iv1_atm * iv1_atm
        v2 = iv2_atm * iv2_atm
        v_fwd = (v2 * t2 - v1 * t1) / (t2 - t1)
        ...
        # 4. Calcular componentes del FF

        # Componente temporal (idéntico al FF clásico si strikes iguales)
        FF_temporal = (iv1_atm / iv_fwd) - 1.0

        # Componente de ajuste por diferencia de strikes
        beta_avg = (beta1 + beta2) / 2.0
        FF_skew = -beta_avg * delta_m / iv_fwd

        # Componente de interacción (term structure del skew)
        dbeta_dt = (beta2 - beta1) / (t2 - t1)
        FF_interaction = -dbeta_dt * t1 * delta_m / iv_fwd

        # 5. FF generalizado total
        FF_gen = FF_temporal + FF_skew + FF_interaction

        # Retornar diccionario con todos los componentes
        return {
            'FF_GEN': round(FF_gen, 6),
            'FF_temporal': round(FF_temporal, 6),
            'FF_skew': round(FF_skew, 6),
            'FF_interaction': round(FF_interaction, 6),
            'iv_fwd': round(iv_fwd, 6),
            'iv1_atm': round(iv1_atm, 6),
            'iv2_atm': round(iv2_atm, 6),
            'delta_moneyness': round(delta_m, 6),
            'beta1_used': round(beta1, 6),
            'beta2_used': round(beta2, 6)
        }

    except Exception as e:
        # En caso de cualquier error, retornar None silenciosamente
        return None


# ================== CÁLCULO ESTRATEGIA ==================
def compute_strategy_metrics(spot,r_base,exp1,dte1,k1,exp2,dte2,k2,k3=None,precache=None):
    ...
    d1,t1,p1,iv1,bid1,ask1,last1 = greeks_price(calls1_idx,exp1,k1,T1,r1,q1)   # -1C@K1
    d2,t2,p2,iv2,bid2,ask2,last2 = greeks_price(calls2_idx,exp2,k2,T2,r2,q2)   # +2C@K2

    dz = lambda x: 0.0 if (x is None or (isinstance(x,float) and math.isnan(x))) else x
    delta_total = -1*dz(d1) + 2*dz(d2)
    theta_total = -1*dz(t1) + 2*dz(t2)

    p1z, p2z = dz(p1), dz(p2)
    net_credit = (-1*p1z) + (2*p2z)  # puntos SPX (CRD: sin pata 3)
    ...
    pnl_total  = pnl_short1 + pnl_long2

    # Death Valley + PnLDV + LEL (Call Ratio Diagonal)
    # Buscar el punto de mínimo P&L entre LEL (S=0) y EarL (S=K1)
    death_valley = None
    pnl_dv_points = None
    LEL = None  # Low Expiration Line (coincide con net_credit en CRD)
    tau = max(T2 - T1, 0.0)

    # LEL: Punto de P&L cuando SPX->0 (ambas calls expiran OTM)
    # En CRD: Si SPX cae a 0, todas las calls expiran OTM (valor=0)
    # PnL = Valor_final - Costo_inicial = 0 - net_credit = -net_credit
    LEL = -net_credit

    iv2_clean = None if (iv2 is None or (isinstance(iv2,float) and math.isnan(iv2))) else float(iv2)

    # Death Valley: punto analítico donde ocurre el mínimo de P&L
    # Para CRD (-1C@K1 + 2C@K2), el valle ocurre donde dPnL/dS = 0
    # Esto sucede cuando el delta de la posición = 0: -1 + 2·Delta_K2 = 0
    # => Delta_K2 = 0.5 => d1 = 0 => S = K2 · exp(-(r + 0.5σ²)τ)
    #
    # IMPORTANTE: Death Valley solo existe si K1 < S0 < K2
    # Si S0 está fuera de este rango, NO hay un verdadero "valle" (el PnL es monotónico)
    if tau > 0 and iv2_clean is not None:
        sigma2 = float(iv2_clean)
        # Calcular el punto analítico donde ocurre el mínimo
        S0 = float(k2) * math.exp(-(r2 + 0.5*sigma2*sigma2) * tau)

        # Death Valley solo es válido si está entre K1 y K2
        # (esto indica que hay un valle real entre las strikes)
        if float(k1) < S0 < float(k2):
            # Calcular PnL en el Death Valley
            # En S0: short call @ K1 pierde (S0 - K1), long 2× calls @ K2 valen 2×BS_Call
            val_short1 = -max(0.0, S0 - float(k1))  # Short call pierde dinero (S0 > K1)
            val_long2 = 2.0 * bs_call_price_safe(S0, float(k2), tau, r2, sigma2)  # Long calls con τ restante
            value_t1 = val_short1 + val_long2
            pnl_dv_points = value_t1 - net_credit
            death_valley = S0
        # Si S0 no está entre K1 y K2, NO reportar Death Valley
        # (death_valley y pnl_dv_points quedan en None)

    # EarL (Oreja izquierda): P&L en K1 al vencimiento T1
    EarL_pts = None
    if tau > 0 and iv2_clean is not None:
        # En K1: short call vence ATM (valor=0), long 2× siguen vivas con τ restante
    ...
    # Calcular FF_ATM (Forward Factor) para CRD
    # Para CRD: ATM_front = IV de K1 (short call), ATM_back = IV de K2 (long call)
    row_id = f"{exp1}/{exp2}|{k1}/{k2}"

    # ATM_front es la IV de la short call (K1)
    atm_front = iv1 if (iv1 is not None and not (isinstance(iv1, float) and math.isnan(iv1))) else None

    # ATM_back es la IV de la long call (K2)
    ...
    # Calcular FF_GEN (Forward Factor Generalizado para strikes diferentes)
    ff_gen_result = compute_FF_GEN(
        IV_front=atm_front,
        IV_back=atm_back,
        K_front=k1,
        K_back=k2,
        ...
        T2_years=T2,
        spot=spot,
        beta1=None,  # Usa default -0.15 (equity near-term)
        beta2=None,  # Usa default -0.10 (equity far-term)
        row_identifier=row_id
    )

    # Extraer valores de FF_GEN (None si fallo el cálculo)
    if ff_gen_result is not None:
        ff_gen_val = ff_gen_result.get('FF_GEN')
        ff_temporal_val = ff_gen_result.get('FF_temporal')
        ff_skew_val = ff_gen_result.get('FF_skew')
        ff_interaction_val = ff_gen_result.get('FF_interaction')
        iv_fwd_gen = ff_gen_result.get('iv_fwd')
    else:
        ff_gen_val = None
        ff_temporal_val = None
        ff_skew_val = None
        ff_interaction_val = None
        iv_fwd_gen = None
    ...
    out = {
        ...
        "FF_GEN": ff_gen_val,
        "FF_temporal": ff_temporal_val,
        "FF_skew": ff_skew_val,
        "FF_interaction": ff_interaction_val,
        "iv_fwd_gen": iv_fwd_gen,
        ...
        "LEL": None if (LEL is None or (isinstance(LEL,float) and math.isnan(LEL))) else round(float(LEL),2),
...
def _process_one_fwd_batman(args):
    ...
    # Extraer fecha base del batman
    try:
        if 'date_us' in row_dict and row_dict['date_us']:
            base_date_str = str(row_dict['date_us'])
        elif 'dia' in row_dict and row_dict['dia']:
            base_date_str = str(row_dict['dia'])
        else:
            result['errors'].append(('fwd_no_base_date', 'No date_us ni dia en row_dict'))
            return result

        base_date = datetime.strptime(base_date_str, "%Y-%m-%d").date()
    except Exception as e:
        result['errors'].append(('fwd_base_date_parse_error', str(e)))
        return result

    filled = 0
    for fr, suf in zip(forward_fracs, frac_suffixes):
        # Calcular cuántos días calendario avanzar (fracción de la vida del batman)
        forward_calendar_days = max(1, int(round(t1_days * fr)))
        if forward_calendar_days >= t1_days:
            continue

        # Calcular fecha objetivo (días calendario desde base)
        target_date = base_date + timedelta(days=forward_calendar_days)
        target_date_str = target_date.strftime("%Y-%m-%d")

        # Buscar archivo que contenga esta fecha en files_sorted_list
        # Implementación tolerante: busca fecha exacta o la más cercana >= target_date (dentro de 5 días)
        fwd_path = None
        min_delta = None
        TOLERANCE_DAYS = 5  # Máximo días de diferencia permitidos

        for fpath in files_sorted_list:
            ...
            # Solo considerar fechas >= target_date (no ir hacia atrás en el tiempo)
            if file_date >= target_date:
                delta = (file_date - target_date).days

                # Límite de tolerancia: máximo TOLERANCE_DAYS días calendario hacia adelante
                if delta <= TOLERANCE_DAYS:
                    if min_delta is None or delta < min_delta:
                        min_delta = delta
                        fwd_path = fpath
                        # Si encontramos match exacto (delta=0), salir inmediatamente
                        if delta == 0:
                            break

        if fwd_path is None or not fwd_path.exists():
            result['errors'].append(('fwd_file_not_found', f'{target_date_str} ± {TOLERANCE_DAYS} days (calendar days: {forward_calendar_days})'))
            continue

        # Log si se usó fecha alternativa (para debugging)
        if min_delta is not None and min_delta > 0:
            result['errors'].append(('fwd_date_adjusted', f'{target_date_str} -> +{min_delta} days'))
        ...
        try:
                ...
                if first_valid_timestamp is None:
                    ...
                    for leg_label, leg_info in (("k1", bv_details.get("leg1")),
                        ...
                        if leg_label == "k1":
                            ...
                            expected_mid = None  # CRD: no existe pata 3
                        mid_status = compare_mid_values(mid_v, expected_mid)

                        # Validación de root (CRD: solo k1 en exp1)
                        if leg_label == "k1":
            ...
            if first_valid_timestamp is not None:
                ...
                # Metadata de las 2 patas
                for leg_label in ("k1", "k2"):
...
def _process_k1_candidate(args):
    ...
    Retorna lista de candidatos CRD generados para este k1.
    """
    (k1, s2_list, e1, e2, dte_map, spot, r_base, date_es_str, hhmm_es, _hhmm_us,
    ...
    # Loop k2 (CRD: solo 2 patas, sin k3)
    for k2 in s2_list:
        ...
        # Prefiltro de crédito (CRD: sin mid3)
        pre_net_credit = (-mid1) + (2*mid2)
        if not (PREFILTER_CREDIT_MIN <= pre_net_credit <= PREFILTER_CREDIT_MAX):
            continue

        # Validar orden de strikes (CRD: solo k1 < k2)
        if not (k1 < k2):
            continue

        # Generar URL
        url = make_url(e1, k1, e2, k2)
        ...
        met = compute_strategy_metrics(
            spot, r_base, e1, dte_map[e1], k1, e2, dte_map[e2], k2, None, precache_local
        )
...
def load_and_clean(params: AnalysisParams) -> Tuple[pd.DataFrame, str]:
    ...
    if params.ratios is None:
        # LISTA EXPLÍCITA DE VARIABLES PARA ANÁLISIS ESTADÍSTICO
        # Solo incluir variables de entrada (métricas), NO resultados forward
        ratio_cols_predefined = [
            'CQI_ABS',      # CRD Quality Indicator Absoluto
            'FF_ATM',       # Forward Factor ATM
            'FF_GEN',       # Forward Factor Generalizado
            'delta_total',  # Delta total de la estructura
            'theta_total',  # Theta total de la estructura
            'net_credit',   # Crédito neto original
            'PnLDV'         # Profit & Loss en Death Valley
        ]

        # Filtrar solo las que existen en el DataFrame
        ratio_cols = [col for col in ratio_cols_predefined if col in df.columns]

        if not ratio_cols:
            raise ValueError("No se encontraron las variables predefinidas en el CSV. Columnas esperadas: " + ", ".join(ratio_cols_predefined))

        logger.info(f"Variables seleccionadas para análisis ({len(ratio_cols)}): {', '.join(ratio_cols)}")
    else:
...
# ================== MÓDULO AVANZADO: MACHINE LEARNING ==================
...
def analyze_regimes(df: pd.DataFrame, ratio_cols: List[str], pnl_col: str, params: AnalysisParams) -> Dict[str, Any]:
    """
    Segmenta el dataset por regímenes (CQI_ABS, FF_ATM, FF_GEN) y analiza correlaciones por régimen.
    ...
    # Régimen 1: CQI_ABS (si existe)
    if 'CQI_ABS' in df.columns:
        cqi_median = df['CQI_ABS'].median()
        high_cqi = df[df['CQI_ABS'] > cqi_median]
        low_cqi = df[df['CQI_ABS'] <= cqi_median]

        if len(high_cqi) >= params.min_n and len(low_cqi) >= params.min_n:
            # Correlaciones en régimen alto CQI
            high_corr = compute_correlation_suite(high_cqi, ratio_cols, pnl_col, params)
            low_corr = compute_correlation_suite(low_cqi, ratio_cols, pnl_col, params)

            regimes['CQI_ABS'] = {
                'high': {'n': len(high_cqi), 'corr_df': high_corr, 'threshold': cqi_median},
                'low': {'n': len(low_cqi), 'corr_df': low_corr, 'threshold': cqi_median}
            }
            logger.info(f"  [REGIMES] CQI_ABS: High n={len(high_cqi)}, Low n={len(low_cqi)}, threshold={cqi_median:.2f}")
    ...
    # Régimen 3: FF_GEN (si existe)
    if 'FF_GEN' in df.columns:
        ff_gen_median = df['FF_GEN'].median()
        high_ff_gen = df[df['FF_GEN'] > ff_gen_median]
        low_ff_gen = df[df['FF_GEN'] <= ff_gen_median]

        if len(high_ff_gen) >= params.min_n and len(low_ff_gen) >= params.min_n:
            high_corr = compute_correlation_suite(high_ff_gen, ratio_cols, pnl_col, params)
            low_corr = compute_correlation_suite(low_ff_gen, ratio_cols, pnl_col, params)

            regimes['FF_GEN'] = {
                'high': {'n': len(high_ff_gen), 'corr_df': high_corr, 'threshold': ff_gen_median},
                'low': {'n': len(low_ff_gen), 'corr_df': low_corr, 'threshold': ff_gen_median}
            }
            logger.info(f"  [REGIMES] FF_GEN: High n={len(high_ff_gen)}, Low n={len(low_ff_gen)}, threshold={ff_gen_median:.2f}")
    ...
    try:
        ...
        for window_idx, pnl_col in enumerate(all_pnl_cols, 1):
            ...
            params = AnalysisParams(
                ...
                ratios=None,  # Auto-detecta todos los ratios disponibles
            ...
            # 3.5. ANÁLISIS AVANZADOS (ML, Regímenes, Reglas, PCA, Temporal)
            logger.info("\nFase 3.5: Ejecutando análisis avanzados...")
            ...
            if len(df) >= 30:  # Mínimo razonable para ML
                try:
                    # Random Forest Feature Importance
                    logger.info("  [ADVANCED] 1/5: Random Forest...")
                    rf_results = analyze_feature_importance_rf(df, ratio_cols, pnl_col_confirmed, params)
                except Exception as e:
                    logger.warning(f"  [ADVANCED] Error en RF: {e}")

                try:
                    # Análisis de Regímenes
                    logger.info("  [ADVANCED] 2/5: Análisis de regímenes...")
                    regime_results = analyze_regimes(df, ratio_cols, pnl_col_confirmed, params)
                except Exception as e:
                    logger.warning(f"  [ADVANCED] Error en regímenes: {e}")

                try:
                    # Búsqueda de Reglas Combinadas
                    logger.info("  [ADVANCED] 3/5: Búsqueda de reglas combinadas...")
                    rules_df = search_rule_combinations(df, ratio_cols, pnl_col_confirmed, params, top_k=20)
                except Exception as e:
                    logger.warning(f"  [ADVANCED] Error en reglas: {e}")

                try:
                    # PCA y Clustering
                    logger.info("  [ADVANCED] 4/5: PCA y clustering...")
                    pca_results = analyze_pca_clustering(df, ratio_cols, pnl_col_confirmed, params, n_clusters=3)
                except Exception as e:
                    logger.warning(f"  [ADVANCED] Error en PCA/clustering: {e}")

                try:
                    # Estabilidad Temporal
                    logger.info("  [ADVANCED] 5/5: Estabilidad temporal...")
                    temporal_results = analyze_temporal_stability(df, ratio_cols, pnl_col_confirmed, date_col, params, n_periods=4)
                except Exception as e:
                    logger.warning(f"  [ADVANCED] Error en análisis temporal: {e}")
            else:
                logger.info(f"  [ADVANCED] Dataset pequeño (n={len(df)}), análisis avanzados omitidos (requiere n>=30)")
            ...
            # Escribir reportes avanzados
            if rf_results or regime_results or not rules_df.empty or pca_results or temporal_results:
                write_advanced_reports(output_dir, pnl_col_confirmed, rf_results, regime_results, rules_df, pca_results, temporal_results)
...
def main():
    print(f"{'='*70}")
    print("CRD V8 BACKTESTER - SISTEMA DE RECUPERACIÓN")
    print(f"{'='*70}")

    # ============================================================
    # DETECCIÓN Y RECUPERACIÓN DE DIRECTORIOS HUÉRFANOS
    # ============================================================
    # Buscar directorios temp_CRD_* en el escritorio
    orphan_dirs = sorted(DESKTOP.glob("temp_CRD_*"), key=lambda p: p.name, reverse=True)
    ...
    if orphan_dirs:
        ...
        for orphan_dir in orphan_dirs:
            ...
            # Si tiene consolidated, puede recuperarse
            if consolidated_file.exists():
                ...
                while True:
                    ...
                    if response == "info":
                        ...
                        temp_dir = orphan_dir
                        ts_batch = orphan_dir.name.replace("temp_CRD_", "")
                        ...
                        batch_out_name = f"CRD_V18_BACKTEST_LOCAL_{ts_batch}_RECOVERED.csv"
                        ...
                        # Inicializar files_sorted para uso posterior
                        files_sorted = list_local_files(DATA_DIR)
                        # Crear lista completa para FWD también en modo recuperación
                        files_sorted_fwd = files_sorted.copy()
                        break
    ...
    if 'RECOVERY_MODE' not in locals() or not RECOVERY_MODE:
        ...
        # Crear copia completa para FWD (ANTES de filtrar por años)
        # Esto permite que el FWD busque en TODOS los archivos disponibles,
        # incluso si YEARS_TO_PROCESS restringe el procesamiento base
        files_sorted_fwd = files_sorted.copy()

        # Aplicar filtro de años si está configurado (solo para selección base)
        if YEARS_TO_PROCESS:
            files_before = len(files_sorted)
            files_sorted = filter_files_by_years(files_sorted, YEARS_TO_PROCESS)
            files_after = len(files_sorted)
            print(f"[i] Filtro de años aplicado: {YEARS_TO_PROCESS}")
            print(f"[i] Archivos para procesamiento base: {files_after}")
            print(f"[i] Archivos disponibles para FWD: {files_before} (sin filtrar)")
            if not files_sorted:
                print(f"[×] No hay archivos que coincidan con los años especificados: {YEARS_TO_PROCESS}")
                return
        else:
            print(f"[i] Sin filtro de años - procesando todos los archivos disponibles: {len(files_sorted)}")
        ...
        batch_out_name = f"CRD_V18_BACKTEST_LOCAL_{ts_batch}.csv"
        batch_out_path = DESKTOP / safe_filename(batch_out_name)

        # Directorio temporal para Parquet incrementales
        temp_dir = DESKTOP / f"temp_CRD_{ts_batch}"
        temp_dir.mkdir(exist_ok=True)
        parquet_files = []

        for pick_idx, chosen_file in enumerate(chosen_files, start=1):
            # Usar índice de la lista completa (files_sorted_fwd) para que el FWD pueda buscar correctamente
            base_idx = files_sorted_fwd.index(chosen_file)
            print(f"[i] Archivo elegido al azar {pick_idx}/{k}: {chosen_file.name} (índice {base_idx} de {len(files_sorted_fwd)} total)")
            ...
            for t_idx, target_hhmm_us in enumerate(TARGET_HHMMS_US if not IGNORE_TARGET_MINUTE else ["ALL"], start=1):
                ...
                print(f"     Combos brutos aprox.: {len(exp_A)}*{len(exp_B)}*|K1|*|K2| (cómputo local)")
                print(f"     Rangos DELTA - K1: [{BASE_P1_DELTA_MIN:.2f}, {BASE_P1_DELTA_MAX:.2f}] | K2: [{P2_DELTA_MIN:.2f}, {P2_DELTA_MAX:.2f}]")

                rows=[]

                # ========== PARALELIZACIÓN DE SELECCIÓN DE CANDIDATOS ==========
                # Preparar todas las tareas de k1 para procesamiento paralelo (CRD: sin k3)
                tasks = []
                for e1 in exp_A:
                    ...
                    s1_list = gen_k1_candidates_by_delta(
                        ...
                        delta_min=BASE_P1_DELTA_MIN,
                        delta_max=BASE_P1_DELTA_MAX
                    )
                    ...
                    for e2 in exp_B:
                        ...
                        s2_list = gen_k2_candidates_by_delta(
                            ...
                            delta_min=P2_DELTA_MIN,
                            delta_max=P2_DELTA_MAX
                        )
                        ...
                        for k1 in s1_list:
                            task = (
                                k1, s2_list, e1, e2, dte_map, spot, r_base,
    ...
    if 'RECOVERY_MODE' not in locals() or not RECOVERY_MODE:
        ...
        # Archivo consolidado temporal (acumulará chunks filtrados)
        consolidated_path = temp_dir / "consolidated_filtered.parquet"
        writer = None
        schema = None
        ...
        for pq_file in parquet_files:
            ...
            # NOTA: UEL_inf_USD eliminado en CRD (no hay pérdida máxima infinita)

            # Aplicar máscara de prefiltros básicos
            df_chunk = df_chunk[mask]
            del mask

            # ========== FASE 2: Calcular CQI_ABS (CRD Quality Indicator) ==========
            # CRD: Solo usa PnLDV y EarL (no hay EarR en CRD)
            if not df_chunk.empty and {"PnLDV","EarL"}.issubset(df_chunk.columns):
                if "CQI_ABS" not in df_chunk.columns:
                    # Constantes para normalización CRD
                    EPS = 1e-6
                    Wv  = 0.6  # Peso de PnLDV (Death Valley)
                    Wl  = 0.4  # Peso de EarL (Oreja izquierda)
                    OFFSET_BASE = 1000.0
                    SCALE_FACTOR = 10.0
                    ...
                    # CQI_ABS para CRD: combinación ponderada de PnLDV (60%) y EarL (40%)
                    # Si PnLDV >= 0: estructura sin pérdida en Death Valley (muy buena)
                    CQI_ABS_pos = OFFSET_BASE + (pnldv / SCALE_FACTOR)

                    # Si PnLDV < 0: balancear EarL vs profundidad del valle
                    CQI_ABS_neg = EL / (EPS + Wv * ValleyDepth)
                    CQI_ABS_neg = np.minimum(CQI_ABS_neg, OFFSET_BASE - 1.0)

                    CQI_ABS = np.where(pnldv >= 0, CQI_ABS_pos, CQI_ABS_neg)

                    df_chunk["CQI_ABS"] = pd.to_numeric(CQI_ABS, errors="coerce")
                    df_chunk["CQI_1000"] = np.rint(df_chunk["CQI_ABS"] * 1000.0).astype("Int64")

                    # Liberar memoria
                    del earL, pnldv, EL, ValleyDepth, CQI_ABS_pos, CQI_ABS_neg, CQI_ABS

            # ========== FASE 3: Filtros avanzados (PnLDV, CQI_ABS) ==========
            ...
            # Filtro CQI_ABS (si está habilitado)
            if FILTER_CQI_ABS_ENABLED and "CQI_ABS" in df_chunk.columns:
                mask_advanced &= df_chunk["CQI_ABS"].notna() & (df_chunk["CQI_ABS"] >= CQI_ABS_MIN) & (df_chunk["CQI_ABS"] <= CQI_ABS_MAX)
            ...
            if not df_chunk.empty:
                # Reset index para evitar conflictos de schema con __index_level_0__
                df_chunk_reset = df_chunk.reset_index(drop=True)
                table = pa.Table.from_pandas(df_chunk_reset)
                if writer is None:
                    # Primera escritura: crear schema y writer
                    schema = table.schema
                    writer = pq.ParquetWriter(consolidated_path, schema)
                writer.write_table(table)
                del table, df_chunk_reset

            del df_chunk
            print(f"  - Procesado: {pq_file.name} ({filas_antes} -> {filas_despues} filas tras prefiltros+CQI_ABS+filtros)")
        ...
        print(f"\n[OK] Consolidacion incremental completada: {total_filas_leidas} filas -> {total_filas_filtradas} filas tras prefiltros")
        print(f"[OK] Reduccion: {100*(1-total_filas_filtradas/total_filas_leidas):.1f}% eliminadas")
        print(f"[OK] Archivo consolidado: {consolidated_path}")

        # Ahora leer SOLO el archivo consolidado (mucho más pequeño)
        print(f"\n[INFO] Cargando DataFrame consolidado filtrado desde disco...")
        df = pd.read_parquet(consolidated_path)
        print(f"[OK] DataFrame cargado: {len(df)} filas")
        ...
        gc.collect()
        print(f"[OK] Archivos temporales eliminados y memoria liberada")

    else:
        # ============================================================
        # MODO RECUPERACIÓN: Cargar archivo consolidado existente
        # ============================================================
        print(f"\n{'='*70}")
        print("MODO RECUPERACIÓN: Cargando archivo consolidado existente")
        print(f"{'='*70}")

        if not consolidated_path.exists():
            print(f"[×] ERROR: No se encontró el archivo consolidado en {consolidated_path}")
            return

        consolidated_size_mb = consolidated_path.stat().st_size / (1024**2)
        print(f"[INFO] Archivo consolidado: {consolidated_size_mb:.1f} MB")
        print(f"[INFO] Cargando DataFrame desde disco...")
    ...
    if not df.empty:
        ...
        print(f"[OK] NET_CREDIT ya prefiltrado: {format_filter_log('NET_CREDIT', PREFILTER_CREDIT_MIN, PREFILTER_CREDIT_MAX, len(df), len(df), stats)}")

        # Verificar DELTA_TOTAL (ya filtrado)
        stats = get_filter_stats(df, "delta_total", DELTA_MIN, DELTA_MAX)
        print(f"[OK] DELTA_TOTAL ya prefiltrado: {format_filter_log('DELTA_TOTAL', DELTA_MIN, DELTA_MAX, len(df), len(df), stats)}")

        # Verificar THETA_TOTAL (ya filtrado)
        stats = get_filter_stats(df, "theta_total", THETA_MIN, THETA_MAX)
        print(f"[OK] THETA_TOTAL ya prefiltrado: {format_filter_log('THETA_TOTAL', THETA_MIN, THETA_MAX, len(df), len(df), stats)}")

        # NOTA: UEL_inf_USD no existe en CRD (estructura sin pérdida máxima infinita)
        ...
        if "PnLDV" in df.columns:
            if FILTER_PNLDV_ENABLED:
                ...
                print(f"[OK] PnLDV ya prefiltrado: {format_filter_log('PnLDV', PNLDV_MIN, PNLDV_MAX, len(df), len(df), stats)}")
            else:
                stats = get_filter_stats(df, "PnLDV", PNLDV_MIN, PNLDV_MAX)
                print(f"[OK] PnLDV [DESACTIVADO]: {format_filter_log('PnLDV', PNLDV_MIN, PNLDV_MAX, len(df), len(df), stats)}")

        # Verificar CQI_ABS (ya filtrado si estaba habilitado)
        if "CQI_ABS" in df.columns:
            if FILTER_CQI_ABS_ENABLED:
                stats = get_filter_stats(df, "CQI_ABS", CQI_ABS_MIN, CQI_ABS_MAX)
                print(f"[OK] CQI_ABS ya prefiltrado: {format_filter_log('CQI_ABS', CQI_ABS_MIN, CQI_ABS_MAX, len(df), len(df), stats)}")
            else:
                stats = get_filter_stats(df, "CQI_ABS", CQI_ABS_MIN, CQI_ABS_MAX)
                print(f"[OK] CQI_ABS [DESACTIVADO]: {format_filter_log('CQI_ABS', CQI_ABS_MIN, CQI_ABS_MAX, len(df), len(df), stats)}")
        else:
            print(f"[!] CQI_ABS no encontrado (no se calculó durante consolidación)")

    # Liberar memoria
    import gc
    gc.collect()
    print(f"\n[OK] Todos los filtros fueron aplicados durante consolidación incremental")
    print(f"[OK] No se requiere filtrado adicional - DataFrame listo para ordenar")

    # Orden interactivo con tie-breakers (usa RANKING_MODE)
    if df.empty:
        print("\nNo hay filas tras filtros; no se aplica orden. CSV vacío.")
    else:
        # Usar RANKING_MODE en lugar de choice hardcoded
        choice = RANKING_MODE

        if choice in ("theta", "t", "θ", "theta_total"):
            df = df.sort_values(by=["theta_total","CQI_ABS","PnLDV"], ascending=[False,False,False])
            used = "theta_total -> CQI_ABS -> PnLDV"
        elif choice in ("delta", "d", "delta_total"):
            df = df.sort_values(by=["delta_total","CQI_ABS","PnLDV"], ascending=[False,False,False])
            used = "delta_total -> CQI_ABS -> PnLDV"
        elif choice in ("dv","death","valley"):
            df = df.sort_values(by=["Death valley","CQI_ABS","PnLDV"], ascending=[True,False,False])
            used = "Death valley -> CQI_ABS -> PnLDV"
        elif choice in ("PnLDV","pnldv","pnl","pnl_dv"):
            df = df.sort_values(by=["PnLDV","CQI_ABS","EarL"], ascending=[False,False,False])
            used = "PnLDV -> CQI_ABS -> EarL"
        elif choice in ("EarL","earl","ear_l"):
            df = df.sort_values(by=["EarL","CQI_ABS","PnLDV"], ascending=[False,False,False])
            used = "EarL -> CQI_ABS -> PnLDV"
        elif choice in ("FF_GEN","ff_gen"):
            df = df.sort_values(by=["FF_GEN","CQI_ABS","PnLDV"], ascending=[False,False,False])
            used = "FF_GEN -> CQI_ABS -> PnLDV"
        elif choice in ("FF_ATM","ff_atm"):
            df = df.sort_values(by=["FF_ATM","CQI_ABS","PnLDV"], ascending=[False,False,False])
            used = "FF_ATM -> CQI_ABS -> PnLDV"
        elif choice in ("bqr","bqi1000","cqr","cqi1000","CQI_1000"):
            df = df.sort_values(by=["CQI_1000","PnLDV","EarL"], ascending=[False,False,False])
            used = "CQI_1000 -> PnLDV -> EarL"
        elif choice in ("CQI_ABS","cqi_abs","bqi","cqi"):
            df = df.sort_values(by=["CQI_ABS","PnLDV","EarL"], ascending=[False,False,False])
            used = "CQI_ABS -> PnLDV -> EarL"
        else:
            # Default: ordenar por CQI_ABS si RANKING_MODE no coincide con ningún caso
            df = df.sort_values(by=["CQI_ABS","PnLDV","EarL"], ascending=[False,False,False])
            used = "CQI_ABS -> PnLDV -> EarL (default)"
        df = df.reset_index(drop=True)
        print(f"Orden aplicado según RANKING_MODE='{RANKING_MODE}': {used}")
    ...
    if not df.empty and (FWD_ON_WINNERS or FWD_ON_LOSERS):
        ...
        if pct_value is None:
            ...
            if FWD_ON_WINNERS and FWD_ON_LOSERS:
                ...
                if N == 0 or total_valid == 0:
                    ...
                    print(f"  - CRDs seleccionados (primeras {N} filas): {num_winners_before}")
                    print(f"  - CRDs tras deduplicación dia+DTE1/DTE2: {len(winner_indices)}")

                    print(f"\n[FWD] LOSERS - Selección por porcentaje posicional {pct_value*100:.1f}%:")
                    print(f"  - Métrica: {RANKING_MODE} (desc)")
                    print(f"  - Total filas válidas: {total_valid}")
                    print(f"  - N calculado: {N} (floor({{pct_value}} × {{total_valid}}))")
                    print(f"  - CRDs seleccionados (últimas {N} filas): {num_losers_before}")
                    print(f"  - CRDs tras deduplicación dia+DTE1/DTE2: {len(loser_indices)}")

                fwd_indices_to_process = winner_indices + loser_indices
                print(f"[FWD] Total CRDs para FWD: {len(fwd_indices_to_process)} ({len(winner_indices)} winners + {len(loser_indices)} losers)")
                ...
                if N == 0 or total_valid == 0:
                    ...
                    print(f"  - Total filas válidas: {total_valid}")
                    print(f"  - N calculado: {N} (floor({{pct_value}} × {{total_valid}}))")
                    print(f"  - CRDs seleccionados (primeras {N} filas): {num_before}")
                    print(f"  - CRDs tras deduplicación dia+DTE1/DTE2: {len(winner_indices)}")

                fwd_indices_to_process = winner_indices
                print(f"[FWD] Calculando forward para {len(fwd_indices_to_process)} CRDs...")
                ...
                if N == 0 or total_valid == 0:
                    ...
                    print(f"  - Total filas válidas: {total_valid}")
                    print(f"  - N calculado: {N} (floor({{pct_value}} × {{total_valid}}))")
                    print(f"  - CRDs seleccionados (últimas {N} filas): {num_before}")
                    print(f"  - CRDs tras deduplicación dia+DTE1/DTE2: {len(loser_indices)}")
        ...
        print(f"Total a procesar: {len(fwd_indices_to_process)} CRDs")
        ...
        if fwd_indices_to_process:
            ...
            for idx in fwd_indices_to_process:
                ...
                args_tuple = (
                    idx,
                    row_dict,
                    files_sorted_fwd,  # ← Usar lista completa para búsqueda FWD
                    FORWARD_FRACS,
                    FRAC_SUFFIXES,
                    str(TZ_US),
                    str(TZ_ES),
            ...
            print(f"  OK Exitosos: {completed_count}")
    ...
    if not df.empty:
        preferred_order = [
            # Columnas principales en orden especificado
            "url", "hora", "hora_us", "root_exp1", "root_exp2",
            "net_credit", "CQI_ABS", "FF_ATM", "FF_GEN", "DTE1/DTE2", "CQI_1000",
            "Death valley", "EarL", "LEL", "PnLDV",
            # Resto de columnas importantes
            "dia", "net_credit_diff",
            # Strikes (CRD: solo 2)
            "k1", "k2",
            # Precios mid (CRD: solo 2 patas)
            "price_mid_short1", "price_mid_long2",
            # IVs
            "iv_k1", "iv_k2",
            # Greeks y valores de entrada
            "delta_total", "theta_total", "net_credit_mediana", "net_credit_mediana_n", "SPX",
            # Ventanas FWD - 01
    ...
    # Exportación (numérico limpio para ordenar bien en CSV) - CRD: sin EarR
    num_cols = ["CQI_ABS","CQI_1000","PnLDV","EarL","LEL","Death valley","delta_total","theta_total","net_credit"]
    for c in num_cols:
        if c in df.columns:
    ...
    if col_filter not in df_copy.columns:
        ...
        if len(df_copy) > 0:
            ...
            def get_base_day_df_cached(dia_str):
                ...
                try:
                    # Buscar archivo correspondiente a dia_str (usar lista completa para no perder datos)
                    matching_files = [f for f in files_sorted_fwd if dia_str in f.name]
            ...
            if len(df_copy) > 0 and FWD_ON_WINNERS and len(winner_indices_copy) > 0:
                ...
                try:
                    # Métricas a correlacionar (solo variables de entrada, no resultados)
                    metrics_to_correlate = [
                        'CQI_ABS', 'FF_ATM', 'FF_GEN',
                    ...
                    print(f"X Error calculando correlaciones: {e}")
                    import traceback
                    traceback.print_exc()
            ...
            columnas_ordenadas = [
                ...
                # Resto de columnas importantes
                "dia", "net_credit_diff",
                # Strikes (CRD: solo k1, k2)
                "k1", "k2",
                # Precios mid (CRD: solo short1, long2)
                "price_mid_short1", "price_mid_long2",
                # IVs (CRD: solo iv_k1, iv_k2)
                "iv_k1", "iv_k2",
                # Greeks y valores de entrada
                "delta_total", "theta_total", "net_credit_mediana", "net_credit_mediana_n", "SPX",
                # Ventanas FWD - 01
            ...
            # ============================================================
            # GRÁFICOS 4A/4B: FILTRADOS POR CQI_ABS > FILTER_CQI_ABS_THRESHOLD
            # ============================================================
            if "CQI_ABS" in df_copy.columns:
                df_bqi_filtered = df_copy[pd.to_numeric(df_copy["CQI_ABS"], errors='coerce') > FILTER_CQI_ABS_THRESHOLD]
                n_bqi = len(df_bqi_filtered)

                if n_bqi > 0:
                    print(f"\n[GRÁFICOS 4A/4B] Filtrado por CQI_ABS > {FILTER_CQI_ABS_THRESHOLD} (n={n_bqi}/{len(df_copy)})")
                    ...
                    if len(pnl_pct_cols) > 0:
                        ...
                        if len(promedios_bqi_pct) > 0:
                            ...
                            ax.set_title(f'Promedio PnL_fwd_pct_mediana por Periodo W\n(Filtrado: CQI_ABS > {FILTER_CQI_ABS_THRESHOLD}, n={n_bqi})', fontsize=13, fontweight='bold')
                            ax.grid(True, alpha=0.3)
                            ax.legend(fontsize=10)
                            plt.tight_layout()

                            plot_name_bqi_pct = batch_out_name.replace(".csv", f"_T0_CQI_ABS_gt{FILTER_CQI_ABS_THRESHOLD}_pct.png")
                            plot_path_bqi_pct = DESKTOP / plot_name_bqi_pct
                            plt.savefig(plot_path_bqi_pct, dpi=150, bbox_inches='tight')
                            plt.close()

                            print(f"  OK [4A] Guardado: {plot_path_bqi_pct}")
                    ...
                    if len(pnl_pts_cols) > 0:
                        ...
                        if len(promedios_bqi_pts) > 0:
                            ...
                            plot_name_bqi_pts = batch_out_name.replace(".csv", f"_T0_CQI_ABS_gt{FILTER_CQI_ABS_THRESHOLD}_pts.png")
                            plot_path_bqi_pts = DESKTOP / plot_name_bqi_pts
                            plt.savefig(plot_path_bqi_pts, dpi=150, bbox_inches='tight')
                            plt.close()

                            print(f"  OK [4B] Guardado: {plot_path_bqi_pts}")
                            print(f"  Valores: {', '.join([f'{labels_bqi_pts[i]}={promedios_bqi_pts[i]:.2f}pts' for i in range(len(promedios_bqi_pts))])}")
                else:
                    print(f"\n[GRÁFICOS 4A/4B] ⚠️ No hay datos con CQI_ABS > {FILTER_CQI_ABS_THRESHOLD}")
            else:
                print(f"\n[GRÁFICOS 4A/4B] ⚠️ Columna 'CQI_ABS' no encontrada")
                ...
                if n_ff > 0:
                    ...
                    if len(pnl_pct_cols) > 0:
                        ...
                        if len(promedios_ff_pct) > 0:
                            ...
                            print(f"  OK [5A] Guardado: {plot_path_ff_pct}")
                    ...
                    if len(pnl_pts_cols) > 0:
                        ...
                        if len(promedios_ff_pts) > 0:
                            ...
                            print(f"  OK [5B] Guardado: {plot_path_ff_pts}")
    ...
    # NOTA: Los archivos individuales ya fueron eliminados durante la consolidación incremental
    # Solo eliminamos el archivo consolidado y el directorio temporal
    print(f"\n[CLEANUP] Limpiando archivos temporales...")

    # Eliminar archivo consolidado si existe
    consolidated_path = temp_dir / "consolidated_filtered.parquet"
    if consolidated_path.exists():
        try:
            consolidated_path.unlink()
            print(f"  - Archivo consolidado eliminado: {consolidated_path.name}")
        except Exception as e:
            print(f"  - Error eliminando archivo consolidado: {e}")

    # Eliminar directorio temporal (debe estar vacío ahora)
    try:
        temp_dir.rmdir()
